...
import org.apache.log4j.{NDC, MDC, Level, Logger}
...
object Log {
   ...
   */
  def forClass[A](implicit mf: Manifest[A]): Log = forClass(mf.erasure)

  /**
   * Returns a log for a given class.
   */
  def forClass(klass: Class[_]): Log = forName(Option(klass.getCanonicalName).getOrElse(klass.getName))
  ...
  /**
   * Set something into the MDC for the duration of function f.
   */
  def mdc[T](name: String, value: Any)(f: => T): T = {
    MDC.put(name, value)
    try { f } finally { MDC.remove(name) }
  }

  /**
   * Push something onto the NDC for the duration of function f.
   */
  def ndc[T](value: String)(f: => T): T = {
    NDC.push(value)
    try { f } finally { NDC.pop() }
  }

  /**
   * NDC is useful, but can easily cause thread leaks. See "known gotchas" here: http://wiki.apache.org/logging-log4j/NDCvsMDC
   * This hack causes log4j to lazily clean up threads which otherwise would leak.  Fun.
   */
  private val ndcCleaner: Thread = {
    val sleepSeconds = 30
